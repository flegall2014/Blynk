...
void Slider::paintEvent(QPaintEvent *event)
{
    ...
    QRect tickRect(sliderPos.x(), rect().height()/2, iSliderWidth, rect().height()/2);
    painter.setPen(QPen(m_majorTickColor));
    painter.drawLine(sliderPos.x(), tickRect.top(), sliderPos.x()+iSliderWidth-1, tickRect.top());
    ...
    {
        // Major tick interval:
        double dMajorTickInterval = (double)ui->wSlider->width()/(double)(nMajorTicks-1);
        ...
        painter.drawLine(sliderPos.x(), tickRect.top()+1, sliderPos.x(), tickRect.top()+1+iMajorTickHeight);
        painter.drawLine(sliderPos.x()+iSliderWidth-1, tickRect.top()+1, sliderPos.x()+iSliderWidth-1, tickRect.top()+1+iMajorTickHeight);

        // Draw other major ticks:
        for (int i=1; i<nMajorTicks-1; i++)
            painter.drawLine(i*dMajorTickInterval+sliderPos.x(), tickRect.top()+1, i*dMajorTickInterval+sliderPos.x(), tickRect.top()+1+iMajorTickHeight);
        ...
        for (int i=0; i<nMajorTicks-1; i++)
        {
            ...
            {
                double x0 = (j+1)*dMinorTickInterval + i*dMajorTickInterval + sliderPos.x();
        ...
        {
            double x0 = i*dMajorTickInterval+sliderPos.x();
